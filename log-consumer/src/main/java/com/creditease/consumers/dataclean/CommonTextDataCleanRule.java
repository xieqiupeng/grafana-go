...
public class CommonTextDataCleanRule implements IDataCleanRule {
        ...
        if (StringUtils.isNotBlank(monitorData)) {
            if (dataCleanRuleEntity.getSeparator() != null) {
                ...
                if (separator.isRegex() != null && separator.isRegex()) {
                    return regexSplit(separator.isOrder(),
                            separator.getSeparatorKeys(),
                            monitorData,
                            dataCleanRuleEntity.getResultColumns());
                } else {
                    return normalSplit(separator.isOrder(),
                            separator.getSeparatorKeys(),
                            monitorData,
                            dataCleanRuleEntity.getResultColumns());
                }
    ...
    private static List<DataCleanEntity> normalSplit(Boolean isOrder,
                                                     List<String> separatorKeys,
                                                     String monitorData,
                                                     List<DataCleanRuleEntity.DataMapping> resultColumns) {
            ...
            if (isOrder != null && isOrder) {
                ...
                separatorKeys.forEach(key -> {
                    if (StringUtils.isNotBlank(key)) {
                        set.add(ESCAPE_CHARACTER.concat(key).trim());
                    }
                });
    ...
    private static List<DataCleanEntity> regexSplit(Boolean isOrder,
                                                    List<String> separatorKeys,
                                                    String monitorData,
                                                    List<DataCleanRuleEntity.DataMapping> resultColumns) {
        List<String> list = new ArrayList<>();
    ...
    private static List<DataCleanEntity> converter(List<String> list,
                                                   List<DataCleanRuleEntity.DataMapping> resultColumns) {
        List<DataCleanEntity> dataCleanEntities = new ArrayList<>();
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        if (map.containsKey(i)) {
                            ...
                            if (StringUtils.isNotBlank(columnName)) {
                                DataCleanEntity entity = new DataCleanEntity();
                                entity.setName(columnName);
                                entity.setValue(DataCleanUtil.getValueByType(list.get(i), mapping.getColumnType(), mapping.getFormat()));
                                entity.setIndex(DataCleanUtil.isTag(mapping.getTagOrValue()));
